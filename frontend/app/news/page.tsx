...
export default async function NewsPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const { items, total } = queryNews({ category: 'all', keyword, limit: 24 });
  ...
  return (
    <>
      ...
      <CategoryTabs />
      <NewsFeed key={keyword || 'all'} category="all" keyword={keyword} initialItems={items} initialTotal={total} hotCoins={hotCoins} dipIndex={dipIndex} topIndex={topIndex} />
    </>
  );
}
